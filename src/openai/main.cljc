(ns openai.main
  "Kotodama WASM entrypoint for the Openai clean-room actor (L4) — Clojure port.
  ...
  facts (`openai.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "openai")
(def tier "L4")
...
(def entity-specs
  [{:entity "Model"           :plural "models"           :id-prefix "openai_mod"
    :fields [:object :created :ownedBy]                  :required [:object]
    :coerce {:created :int}                  :refs {}}
   {:entity "FineTuningJob"   :plural "finetuningjobs"   :id-prefix "openai_fin"
    :fields [:model :status :fineTunedModel :finishedAt] :required [:model]
    :coerce {:finishedAt :int}                :refs {}}
   {:entity "Batch"           :plural "batches"          :id-prefix "openai_bat"
    :fields [:object :endpoint :model :status :inputFileId] :required [:object]
    :coerce {}                                :refs {}}
   {:entity "File"            :plural "files"            :id-prefix "openai_fil"
    :fields [:bytes :filename :object :purpose :status]  :required [:bytes]
    :coerce {:bytes :int}                     :refs {}}
   {:entity "Embedding"       :plural "embeddings"       :id-prefix "openai_emb"
    :fields [:index :object]                             :required [:index :object]
    :coerce {:index :int}                     :refs {}}
   {:entity "ChatCompletion"  :plural "chatcompletions"  :id-prefix "openai_cha"
    :fields [:model :object :created]                    :required [:model]
    :coerce {:created :int}                   :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"openai.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "openai-compat" :tier tier :entities entities} 200])
...
(defn start! [] :openai-compat/ready)
